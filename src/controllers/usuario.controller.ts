...
export class UsuarioController {
  ...
  @response(200, {
    description: "identificar usuario, correo y clave",
    content: {'application/json':{schema: getModelSchemaRef(Usuario)}}
  })
  async identificarUsuario(
    ...
    if (usuario){
      ...
      usuario.clave = "";
      //notificar al usuario via email o sms
      return usuario;
  ...
  async verificarCodigo2fa(
    ...
    if (usuario) {
      ...
      console.log("no se ha almacenado el estado del cambio del token en base de datos")
    }
      return {
        user: usuario,
        token: token
